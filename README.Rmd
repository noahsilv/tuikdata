...
```{r statistical-example, eval = FALSE}
library(tuikr)
...
`statistical_resources()` returns the full supported portal catalog for a
theme: SDMX dataflows, direct file downloads, legacy databases, press
...
`dataflow_id` is the canonical machine identifier for SDMX-backed datasets.
Use `statistical_data()` to download observations. Some datasets work with the
default `key = "ALL"`, while others need a more specific SDMX key.
