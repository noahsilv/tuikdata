...
The planned SDMX layer in this rewrite centers on `statistical_data_structure()`
for metadata and `statistical_data()` for downloads. Both entry points will use
`dataflow_id` values returned by `statistical_tables()` once they are added.
