...
read_xls("/tmp/data.xls") |> dplyr::slice(1:5)
read_xls("/tmp/data.xls") |> dplyr::slice((dplyr::n()-5):dplyr::n())
```
...
require a dataset-specific key.

The safest workflow is:

1. Discover a `dataflow_id` with `statistical_tables()`.
2. Start with `statistical_data()` and the default `key = "ALL"`.
3. If the dataset needs a narrower query, adjust the key based on the TUIK SDMX conventions for that dataset.
...
var_info <- vars |>
  dplyr::filter(.data$var_num == "TFE-GK105747-O23001")
...
download_if_available <- function(var_no, level, ...) {
  ...
  available_levels <- vars |>
    dplyr::filter(.data$var_num == var_no) |>
    dplyr::pull(.data$var_levels) |>
    purrr::pluck(1)
...
data <- geo_data(
  ...
  dplyr::mutate(
    ...
    month = dplyr::if_else(nchar(date) == 6, as.integer(substr(date, 5, 6)), NA_integer_)
  )
```
